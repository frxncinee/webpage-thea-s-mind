...
function Question() {
  ...
  const trends = ['Why is my gf so pretty?', 'Movies to watch with my gf', `corny jokes for my gf`,]

  const handleSearch = (e) => {
    e.preventDefault()
    if (searchQuery.toLowerCase() === 'when is valentines?' || searchQuery.toLowerCase() === 'when is valentines?') {
          navigate("/timer");
       ...
       <div
        ...
        <div className="flex flex-col min-h-screen w-full max-w-[480px] mx-auto ">
          ...
          <div className='mt-10  px-5 text-white/50'>
            <p>Hint: try searching "when is valentines?"</p>
          </div>
        </div>
      )}
    </div>
